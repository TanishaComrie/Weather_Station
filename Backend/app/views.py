...
@app.route('/api/mmar/heat_index/<start>/<end>', methods=['GET'])
def get_heat_index_mmar(start, end):
        ...
        if request.method == "GET":
            try:
                item= mongo.heat_indexMMAR(start, end)
                if item:
                    return jsonify({"status":"found","data":item})
...
@app.route('/api/mmar/soil_moisture/<start>/<end>', methods=['GET'])
def get_soil_moisture_mmar(start, end):
        '''RETURNS MIN, MAX, AVG AND RANGE FOR SOIL MOISTURE PERCENT. THAT FALLS WITHIN THE START AND END DATE RANGE'''
        start= int(start)
        end= int(end)

        if request.method == "GET":
            try:
                item= mongo.soil_moistureMMAR(start, end)
                if item:
                    return jsonify({"status":"found","data":item})
...
def getAllInRange(self,start, end):
        ...
        try:
            ...
            result      = list(remotedb.ELET2415.weather_station.find(query, projection=projection).sort(sort))
        except Exception as e:
            msg = str(e)
