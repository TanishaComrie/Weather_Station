...
class DB:
    ...
    def getAllInRange(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list (remotedb.ELET2415.weather_station.find({"timestamp": {"$gte":start,"$lte":end}},{"_id":0}).sort("timestamp",1).limit(20))
        except Exception as e:
            msg = str(e)
    ...
    def humidityMMAR(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weather_station.aggregate({"$match":{{"timestamp": {"$gte","$lte"}}},"$project":{"$humidity":"humidity"," _id":0}}))
        except Exception as e:
            msg = str(e)
    ...
    def temperatureMMAR(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weather_station.aggregate({"$match":{{"timestamp": {"$gte","$lte"}}},"$project":{"$temperature":"temperature"," _id":0}}))
        except Exception as e:
            msg = str(e)
    ...
    def pressureMMAR(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weather_station.aggregate({"$match":{{"timestamp": {"$gte","$lte"}}},"$project":{"$pressure":"pressure"," _id":0}}))
        except Exception as e:
            msg = str(e)
    ...
    def soil_moistureMMAR(self,start, end):
        '''RETURNS MIN, MAX, AVG AND RANGE FOR SOIL MOISTURE. THAT FALLS WITHIN THE START AND END DATE RANGE'''
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weather_station.aggregate({"$match":{{"timestamp": {"$gte","$lte"}}},"$project":{"$soil_moisture":"soil_moisture"," _id":0}}))
        except Exception as e:
            msg = str(e)
            print("pressureMMAS error ",msg)            
        else:                  
    ...
    def heat_indexMMAR(self,start, end):
        '''RETURNS MIN, MAX, AVG AND RANGE FOR HEAT INDEX. THAT FALLS WITHIN THE START AND END DATE RANGE'''
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weather_station.aggregate({"$match":{{"timestamp": {"$gte","$lte"}}},"$project":{"$heat_index":"heat_index"," _id":0}}))
        except Exception as e:
            msg = str(e)
            print("heat_indexMMAS error ",msg)            
        else:                  
            return result
    ...
    def altitudeMMAR(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weather_station.aggregate({"$match":{{"timestamp": {"$gte","$lte"}}},"$project":{"$altitude":"altitude"," _id":0}}))
        except Exception as e:
            msg = str(e)
            print("altitudeMAS error ",msg)            
        else:                  
            return result
    ...
    def frequencyDistro(self,variable,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.weather_station.aggregate([{'$match': {'timestamp': { '$gte': start, '$lte': end}}},{'$bucket': {'groupBy': f"${variable}",'boundaries': [0, 20, 40, 60, 80, 100],'default': 'outliers','output': {'count': { '$sum': 1 }}}}]))
        except Exception as e:
            msg = str(e)
